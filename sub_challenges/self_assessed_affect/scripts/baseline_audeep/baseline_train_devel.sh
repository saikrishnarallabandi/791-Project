...
#for C in $C_range; do
	...
	java -Xmx$jvm_mem -classpath "$weka_jar" weka.classifiers.meta.FilteredClassifier -v -o -no-cv -c $lab_nominal -t "$train_arff_up" -d "$svm_model_name" -F "weka.filters.unsupervised.attribute.Remove -R 1,2,$lab_numeric" -W weka.classifiers.functions.SMO -- -C $C -L $L -N 1 -M -P 1.0E-12 -V -1 -W 1 -K "weka.classifiers.functions.supportVector.PolyKernel -C 250007 -E 1.0" || exit 1
	fi
